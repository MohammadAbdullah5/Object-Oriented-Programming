    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            do
            {
                ...
                if (option == "1")
                {
                    ...
                    role = login(username, password, usersCount, users);
                ...
                if (option == "2")
                {
                    Credentials fans = new Credentials();
                    Console.Write("Enter Username: ");
                    fans.username = Console.ReadLine();
                    Console.Write("Enter Password: ");
                    fans.password = Console.ReadLine();
                    fans.role = "Fans";
                    users.Add(fans);
                }
            }
        ...
        static void adminInterface(ref int usersCount, ref int currentCoaches, ref int currentPlayers, List<Credentials> users, List<Player> players, List<Coach> coaches, List<string> achievements, ref int ticketPrices, List<MatchSchedule> matchSchedules)
        {
            ...
            do
            {
                ...
                {
                    header();
                    addPlayer(ref currentPlayers, players);
                }

                else if (adminOption == "2")
                {
                    header();
                    removePlayer(ref currentPlayers, players);
                }
        ...
        static string login(string name, string password, int usersCount, List<Credentials> users)
        {
            for (int idx = 0; idx < users.Count; idx++)
            {
                if ((name == users[idx].username) && (password == users[idx].password))
                {
        ...
        static void loadData(ref int count, List<Credentials> users)
        {
            ...
            while ((line = file.ReadLine()) != null)
            {
                ...
                info.username = parseWord(line, 1);
                info.password = parseWord(line, 2);
                info.role = parseWord(line, 3);
                users.Add(info);
                count++;
            }
            file.Close();
        ...
        static void removePlayer(ref int currentPlayers, List<Player> players)
        {
            string removingPlayer;
            Console.Write("Enter the name of Player: ");
            removingPlayer = validateString(Console.ReadLine());
            ...
            while (userEnter != "0")
            {
                ...
                for (int idx = 0; idx < players.Count; idx++)
                {
                    if (search == players[idx].name)
                    {
                        ...
                        Console.WriteLine("       Player's Role: {0}", players[idx].role);
                        Console.WriteLine("       Player's Batting Style: {0}", players[idx].battingStyle);
                        Console.WriteLine("       Player's Bowling Style: {0}", players[idx].bowlingStyle);
                        Console.WriteLine("       Player's Salary: {0}", players[idx].salary);
                    }
                }
                if (flag == false)
                {
